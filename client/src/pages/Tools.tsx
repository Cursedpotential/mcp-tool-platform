// File: client/src/pages/Tools.tsx | Date: 2026-01-11 | Agent: Claude Code | Model: Opus 4.1
...
> = {
  search: {
    ...
    description: "Text search with ripgrep/ugrep",
  },
  document: {
    icon: <FileText className="h-5 w-5" />,
    color: "bg-green-500/10 text-green-500",
    description: "OCR, conversion, chunking",
  },
  nlp: {
    icon: <Brain className="h-5 w-5" />,
    color: "bg-purple-500/10 text-purple-500",
    description: "Entity extraction, sentiment, keywords",
  },
  diff: {
    icon: <GitCompare className="h-5 w-5" />,
    color: "bg-orange-500/10 text-orange-500",
    description: "Text comparison and merging",
  },
  filesystem: {
    icon: <FolderTree className="h-5 w-5" />,
    color: "bg-yellow-500/10 text-yellow-500",
    description: "File operations with approval",
  },
  ml: {
  ...
  rules: {
    ...
    description: "Pattern matching and actions",
  },
  summarization: {
  ...
  retrieval: {
    ...
    description: "BM25 and span retrieval",
  },
};
...
export default function Tools() {
  ...
  const { data: searchResults, isLoading: searchLoading } =
    trpc.mcp.searchTools.useQuery(
      { query: searchQuery || "*", topK: 100 },
      { enabled: true }
    );

  const { data: toolSpec, isLoading: specLoading } =
    trpc.mcp.describeTool.useQuery(
  ...
  const invokeTool = trpc.mcp.invokeTool.useMutation({
    onSuccess: result => {
      setTestResult({
        ...
        latencyMs:
          Date.now() -
          (window as unknown as { __toolStartTime?: number }).__toolStartTime!,
      });
      setIsRunning(false);
        ...
        <div className="flex items-center gap-2">
          <Switch
            ...
            onCheckedChange={checked =>
              handleParamChange(name, checked, schema)
            }
          />
  ...
  const tools =
    (searchResults as unknown as { data?: { tools?: ToolItem[] } })?.data
      ?.tools || [];
  const toolsByCategory = tools.reduce(
    (acc: Record<string, ToolItem[]>, tool: ToolItem) => {
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Browse, search, and test preprocessing tools
          </p>
        </div>
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-6">
              <CardHeader>
                <CardTitle>
                  {selectedTool
                    ? selectedTool.split(".").slice(1).join(".")
                    : "Select a Tool"}
                </CardTitle>
                {selectedTool && (
                ...
                {!selectedTool ? (
                  ...
                  <div className="space-y-4">
                    <p className="text-sm text-muted-foreground">
                      {toolSpec.data?.description || "No description"}
                    </p>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        {(() => {
                          const props = toolSpec.data?.inputSchema?.properties;
                          const required =
                            toolSpec.data?.inputSchema?.required || [];
                          if (!props)
                            return (
                              <p className="text-sm text-muted-foreground">
                                No parameters
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {toolSpec?.data?.inputSchema?.properties ? (
                Object.entries(toolSpec.data.inputSchema.properties).map(
                  ([name, schema]) => {
                    const isRequired =
                      toolSpec.data?.inputSchema?.required?.includes(name) ||
                      false;
                    return (
