...
import { getDb, testConnection, checkPgVector, checkPostGIS, checkExtensions } from "../../core/db.postgres";
import { encryptApiKey, decryptApiKey, maskApiKey, hashApiKey } from "../../core/encryption";
...
interface NlpConfig {
  ...
  chunkingStrategy: 'fixed_size' | 'semantic' | 'sliding_window' | 'conversation_turn' | 'paragraph';
  chunkSize: number;
  chunkOverlap: number;
}
...
async function saveApiKeyToDb(userId: number, providerName: string, encryptedKey: string, baseUrl?: string): Promise<number> {
  const db = await getDb();
...
export const settingsRouter = router({
  ...
  getDatabaseConfig: protectedProcedure.query(async ({ ctx }): Promise<DatabaseConfig> => {
  ...
  addApiKey: protectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<{ success: boolean; keyId?: number; error?: string }> => {
  ...
  updateApiKey: protectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        console.log(`[Settings] Updated API key ${input.id} for user ${userId}`);
  ...
  deleteApiKey: protectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        console.log(`[Settings] Deleted API key ${input.id} for user ${userId}`);
  ...
  getNlpConfig: protectedProcedure.query(async ({ ctx }): Promise<NlpConfig> => {
  ...
  updateNlpConfig: protectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        console.log(`[Settings] Updated NLP config for user ${userId}:`, input);
  ...
  getWorkflowConfig: protectedProcedure.query(async ({ ctx }): Promise<WorkflowConfig> => {
  ...
  updateWorkflowConfig: protectedProcedure
    ...
    .mutation(async ({ ctx, input }): Promise<{ success: boolean; error?: string }> => {
      ...
      try {
        ...
        // In production, save to database
        console.log(`[Settings] Updated workflow config for user ${userId}:`, input);
