...
export const analysisModules = mysqlTable("analysisModules", {
	...
	updatedAt: datetime({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
},
(table) => [
...
export const apiKeys = mysqlTable("apiKeys", {
	...
	lastUsedAt: datetime({ mode: 'string' }),
...
export const hurtlexSyncStatus = mysqlTable("hurtlexSyncStatus", {
	...
	lastSyncAt: datetime({ mode: 'string' }),
	termCount: int().default(0).notNull(),
	sourceUrl: text(),
...
export const users = mysqlTable("users", {
	...
	createdAt: datetime({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: datetime({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
	lastSignedIn: datetime({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
},
(table) => [
