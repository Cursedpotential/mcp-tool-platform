...
export class FacebookHTMLParser {
  ...
  async parse(filePath: string): Promise<FacebookMessage[]> {
    ...
    for await (const line of rl) {
      ...
      if (
        line.includes('<div class="pam _3-95') ||
  ...
  private parseMessageHTML(html: string): FacebookMessage | null {
    try {
      ...
      // Extract sender
      const senderEl =
        root.querySelector('[data-testid="messenger_username"]') ||
      ...
      const textEl =
        root.querySelector('[data-testid="messenger_message_text"]') ||
      ...
      // Extract timestamp
      const timestampEl =
        root.querySelector('[data-testid="messenger_timestamp"]') ||
        root.querySelector("._3-94._2lem");
      const timestampStr = timestampEl?.text?.trim();
      const timestamp = timestampStr
        ? this.parseTimestamp(timestampStr)
        : new Date();
      ...
      if (
        root.querySelector('[data-testid="photo"]') ||
        html.includes("photo.jpg")
      ) {
        messageType = "photo";
      } else if (
        root.querySelector('[data-testid="video"]') ||
        html.includes("video.mp4")
      ) {
        messageType = "video";
      } else if (
        root.querySelector('[data-testid="audio"]') ||
        html.includes("audio.mp3")
      ) {
        messageType = "audio";
      }
