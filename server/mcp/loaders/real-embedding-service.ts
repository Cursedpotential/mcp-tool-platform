...
export class RealEmbeddingService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    console.log(
      `[RealEmbeddingService] Generating embedding (${text.length} chars)`
    );
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        console.error(
          `[RealEmbeddingService] Attempt ${attempt}/${this.maxRetries} failed:`,
          error.message
        );
  ...
  async generateEmbeddings(
    ...
    console.log(
      `[RealEmbeddingService] Generating ${texts.length} embeddings (batch size: ${batchSize})`
    );
    ...
    for (let i = 0; i < texts.length; i += batchSize) {
      ...
      console.log(
        `[RealEmbeddingService] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(texts.length / batchSize)}`
      );
      ...
      try {
        ...
        console.error(
          `[RealEmbeddingService] Batch failed, falling back to sequential:`,
          error.message
        );
        ...
        for (const text of batch) {
          try {
            ...
            console.error(
              `[RealEmbeddingService] Sequential embedding failed:`,
              seqError.message
            );
  ...
  private async generateBatch(texts: string[]): Promise<number[][]> {
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        console.error(
          `[RealEmbeddingService] Batch attempt ${attempt}/${this.maxRetries} failed:`,
          error.message
        );
  ...
  async generateEmbeddingsParallel(
    ...
    console.log(
      `[RealEmbeddingService] Generating ${texts.length} embeddings (parallel, concurrency: ${concurrency})`
    );
    ...
    for (let i = 0; i < texts.length; i++) {
      const promise = (async (index: number) => {
        try {
          ...
          console.error(
            `[RealEmbeddingService] Parallel embedding ${index} failed:`,
            error.message
          );
