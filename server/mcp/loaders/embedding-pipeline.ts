...
export class EmbeddingService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    console.log(
      "[EmbeddingService] Generating embedding for text:",
      text.slice(0, 50) + "..."
    );
  ...
  async generateEmbeddings(texts: string[]): Promise<number[][]> {
    console.log(
      `[EmbeddingService] Generating ${texts.length} embeddings in batch`
    );
...
export class VectorStore {
  ...
  async deleteDocumentEmbeddings(documentId: string): Promise<void> {
    console.log(
      `[VectorStore] Deleting embeddings for document: ${documentId}`
    );
...
export class EmbeddingPipeline {
  ...
  async processDocument(
    ...
    console.log(
      `[EmbeddingPipeline] Processing document: ${document.id} (${chunks.length} chunks)`
    );
    ...
    console.log(
      `[EmbeddingPipeline] Stored ${vectors.length} embeddings for ${document.id}`
    );
  }
...
export async function findSimilarMessages(
  ...
  return results
    .filter(r => r.document_id !== excludeDocumentId)
    .slice(0, topK);
}
