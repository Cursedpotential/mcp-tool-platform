...
describe("CommunicationPatternAnalyzer", () => {
  ...
  describe("BUILT_IN_MODULES", () => {
    it("should have both negative and positive modules", () => {
      const negativeModules = BUILT_IN_MODULES.filter(
        m => m.category === "negative"
      );
      const positiveModules = BUILT_IN_MODULES.filter(
        m => m.category === "positive"
      );
    ...
    it("should have MCL factors assigned to relevant modules", () => {
      const modulesWithMcl = BUILT_IN_MODULES.filter(
        m => m.mclFactors && m.mclFactors.length > 0
      );
  ...
  describe("BUILT_IN_PATTERNS", () => {
    ...
    it("should have patterns for blame_shifting", () => {
      ...
      expect(BUILT_IN_PATTERNS.blame_shifting.patterns).toContain(
        "this is your fault"
      );
    });
  });
  ...
  describe("analyze", () => {
    it("should detect gaslighting patterns", async () => {
      const text =
        "that never happened, you're imagining things. you're being paranoid.";
      const result = await analyzer.analyze(text);

      expect(result.negativeMatches.length).toBeGreaterThan(0);
      const gaslightingMatches = result.negativeMatches.filter(
        m => m.moduleId === "gaslighting"
      );
    ...
    it("should detect love bombing patterns", async () => {
      const text =
        "You're perfect, you're my everything. I've never felt this way about anyone.";
      const result = await analyzer.analyze(text);

      expect(result.positiveMatches.length).toBeGreaterThan(0);
      const loveBombingMatches = result.positiveMatches.filter(
    ...
    it("should detect blame shifting patterns", async () => {
      const text =
        "This is your fault for not listening to me. You made me act this way.";
      const result = await analyzer.analyze(text);

      const blameMatches = result.negativeMatches.filter(
        m => m.moduleId === "blame_shifting"
      );
    ...
    it("should detect threats and intimidation", async () => {
      const text =
        "You'll regret this decision. You'll never see the kids again.";
      const result = await analyzer.analyze(text);

      const threatMatches = result.negativeMatches.filter(
        m => m.moduleId === "threats_intimidation"
      );
    ...
    it("should calculate severity score", async () => {
      const text =
        "That never happened. You're crazy. This is your fault. You'll regret this.";
      const result = await analyzer.analyze(text);
    ...
    it("should include context around matches", async () => {
      const text =
        "Before the incident, she said that never happened and then walked away.";
      const result = await analyzer.analyze(text, {
        includeContext: true,
        contextChars: 50,
      });
    ...
    it("should track MCL factor scores", async () => {
      const text =
        "You'll never see the kids again. Your father doesn't love you.";
      const result = await analyzer.analyze(text);
    ...
    it("should generate a summary", async () => {
      const text =
        "You're perfect! But that never happened, you're imagining things.";
      const result = await analyzer.analyze(text);
    ...
    it("should detect contradictions between positive and negative patterns", async () => {
      const text =
        "you're my soulmate, i love you so much. but you're crazy and that never happened.";
      const result = await analyzer.analyze(text);
    ...
    it("should build a timeline of events", async () => {
      const text =
        "You're perfect. Then later, that never happened. Finally, I'm sorry.";
      const result = await analyzer.analyze(text);
  ...
  describe("apologies detection", () => {
    it("should detect apology patterns", async () => {
      const text =
        "I'm sorry, I was wrong. Please forgive me, I made a mistake.";
      const result = await analyzer.analyze(text);

      const apologyMatches = result.positiveMatches.filter(
        m => m.moduleId === "apologies"
      );
  ...
  describe("parental alienation detection", () => {
    it("should detect parental alienation patterns", async () => {
      const text =
        "your father doesn't love you. daddy doesn't care about you anymore.";
      const result = await analyzer.analyze(text);

      const alienationMatches = result.negativeMatches.filter(
        m => m.moduleId === "parental_alienation"
      );
