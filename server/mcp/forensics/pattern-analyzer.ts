...
export const BUILT_IN_MODULES: AnalysisModule[] = [
  // NEGATIVE PATTERNS - Manipulation & Control
  {
    ...
    description:
      "Identifies reality-distorting statements that make the victim question their perception",
    category: "negative",
  ...
  },
  {
    ...
    name: "Blame Shifting",
    description:
      "Detects patterns where responsibility is deflected onto the victim",
    category: "negative",
    subcategory: "manipulation",
    enabled: true,
    weight: 85,
  ...
  },
  {
    ...
    description:
      "Identifies attempts to downplay concerns, feelings, or events",
    category: "negative",
    subcategory: "manipulation",
    enabled: true,
    weight: 75,
  ...
  },
  {
    ...
    description:
      "Detects explicit or implicit threats, ultimatums, and intimidating language",
    category: "negative",
    subcategory: "control",
    enabled: true,
    weight: 95,
  ...
  },
  {
    ...
    description:
      "Detects withdrawal, refusal to communicate, and silent treatment patterns",
    category: "negative",
    subcategory: "control",
    enabled: true,
    weight: 70,
  ...
  // POSITIVE PATTERNS - Love Bombing & Affection
  {
    ...
    name: "Love Bombing",
    description:
      "Identifies excessive flattery, premature declarations of love, and overwhelming affection",
    category: "positive",
  ...
  },
  {
    ...
    name: "Future Faking",
    description:
      "Detects grandiose promises about the future that may not be fulfilled",
    category: "positive",
  ...
  },
  {
    ...
    description:
      "Identifies apologies and expressions of remorse (to track sincerity over time)",
    category: "positive",
  ...
  },
  {
    ...
    description:
      "Tracks mentions of gifts, financial generosity, and material expressions of love",
    category: "positive",
    subcategory: "affection",
    enabled: true,
    weight: 55,
  ...
  // NEUTRAL PATTERNS - Context & Metadata
  {
    ...
    description:
      "Identifies discussions about custody schedules, visitation, and parenting time",
    category: "neutral",
  ...
  },
  {
    ...
    description:
      "Tracks references to children's health, education, and emotional state",
    category: "neutral",
...
export class CommunicationPatternAnalyzer {
  ...
  constructor() {
    ...
    this.modules = BUILT_IN_MODULES.map(m => ({
      ...m,
      mclFactors: [...(m.mclFactors || [])],
    }));
  }
  ...
  async loadUserConfig(userId: number): Promise<void> {
    ...
    for (const p of userPatterns) {
      const moduleId =
        p.category?.toLowerCase().replace(/\s+/g, "_") || "custom";
      const existing = this.customPatterns.get(moduleId) || {
        patterns: [],
        examples: [],
      };
  ...
  async analyze(
    ...
    const { moduleIds, includeContext = true, contextChars = 100 } = options;
    ...
    return {
      ...
      totalMatches:
        negativeMatches.length + positiveMatches.length + neutralMatches.length,
      negativeMatches,
      positiveMatches,
      neutralMatches,
      severityScore,
  ...
  private calculateSeverityScore(negativeMatches: PatternMatch[]): number {
    ...
    const countMultiplier = Math.min(
      2,
      1 + Math.log10(negativeMatches.length + 1)
    );
  ...
  private detectContradictions(
    ...
    const loveBombing = positiveMatches.filter(
      m => m.moduleId === "love_bombing"
    );
    ...
    for (const positive of loveBombing) {
      for (const negative of devaluation) {
        ...
        const distance = Math.abs(
          positive.position.start - negative.position.start
        );
        if (distance < 2000) {
          contradictions.push({
  ...
  private generateSummary(
    ...
    if (negativeMatches.length > 0) {
      ...
      parts.push(
        `Detected ${negativeMatches.length} concerning patterns, primarily: ${topNegative.join(", ")}.`
      );
    }
    ...
    if (positiveMatches.length > 0) {
      ...
      parts.push(
        `Found ${positiveMatches.length} positive/affectionate patterns: ${topPositive.join(", ")}.`
      );
    }

    if (contradictions.length > 0) {
      parts.push(
        `Identified ${contradictions.length} potential contradictions between positive and negative statements.`
      );
    }

    if (severityScore > 0) {
      const level =
        severityScore >= 80 ? "HIGH" : severityScore >= 50 ? "MODERATE" : "LOW";
