...
import {
  FacebookHTMLParser,
  FacebookMessage,
} from "../loaders/facebook-parser";
import { XMLSmsParser, SmsMessage } from "../loaders/xml-sms-parser";
import {
  PDFImessageParser,
  ImessageMessage,
} from "../loaders/pdf-imessage-parser";
...
export class EndToEndPipeline {
  ...
  async process(options: PipelineOptions): Promise<ProcessedMessage[]> {
    ...
    onProgress?.(
      30,
      100,
      `Found ${messages.length} messages. Starting analysis...`
    );
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (i % 100 === 0) {
        ...
        onProgress?.(
          progress,
          100,
          `Analyzing message ${i + 1}/${messages.length}...`
        );
      }
      ...
      const yearMonth =
        message.timestamp.getFullYear().toString().slice(-2) +
      ...
      processedMessages.push({
        ...
        preliminaryConfidence: Math.round(
          classification.sentiment.confidence * 100
        ),
