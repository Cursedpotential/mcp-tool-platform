...
export class ProductionPipeline {
  ...
  async processDocument(
    ...
    try {
      ...
      const messages = await this.parseDocument(
        filePath,
      ...
      for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex++) {
        ...
        const classifiedMessages = await this.classifyChunk(
          chunk,
          options.userId
        );
        ...
        const conversations = this.groupIntoConversations(
          classifiedMessages,
          documentRecord
        );
   ...
   * Step 4a: Store chunk in Chroma (working memory)
   */
  private async storeChunkInChroma(
    chunk: any[],
    documentId: string,
    chunkIndex: number
  ): Promise<void> {
  ...
  private async insertIntoSupabase(
    ...
    for (const conversation of conversations) {
      // Insert conversation
      const { data: convData, error: convError } = await supabaseManager[
        "client"
      ]
      ...
      // Insert messages
      const messageRecords = conversation.messages.map(
        (msg: any, idx: number) => ({
      ...
      const behaviorRecords = allBehaviors
        .filter(b =>
          conversation.messages.some((m: any) => m.text === b.messageText)
        )
        .map(b => {
   ...
   * Step 6: Insert entities into Neo4j/Graphiti
   */
  private async insertEntitiesIntoNeo4j(
    entities: any[],
    documentId: string
  ): Promise<void> {
  ...
  private async scheduleChromaCleanup(documentId: string): Promise<void> {
    ...
    console.log(
      `Chroma cleanup scheduled for document ${documentId} (72hr TTL)`
    );
  }
