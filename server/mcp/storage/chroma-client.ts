...
export class ChromaManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `  - Evidence: ${this.EVIDENCE_COLLECTION} (${this.TTL_HOURS}hr TTL)`
      );
  ...
  async addEvidence(
    ...
    console.log(
      `[ChromaManager] Adding ${chunks.length} evidence chunks (expires: ${expiresAt.toISOString()})`
    );
  ...
  async updateEvidenceClassification(
    ...
    console.log(
      `[ChromaManager] Updating classification for chunk: ${chunkId}`
    );
  ...
  async cleanupExpiredEvidence(): Promise<number> {
    ...
    console.log(
      `[ChromaManager] Cleaned up ${expired.ids.length} expired evidence chunks`
    );
  ...
  async getEvidenceStats(): Promise<{
    ...
    for (let i = 0; i < all.ids.length; i++) {
      ...
      if (meta?.platform) {
        stats.by_platform[meta.platform] =
          (stats.by_platform[meta.platform] || 0) + 1;
      }
