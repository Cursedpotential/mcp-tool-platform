...
export class SupabaseMessageExporter {
  ...
  async export(messages: ProcessedMessage[]): Promise<ExportResult> {
    ...
    for (const [platform, platformMessages] of Object.entries(
      messagesByPlatform
    )) {
      const platformResult = await this.exportPlatform(
        platform,
        platformMessages
      );
  ...
  ): Record<string, ProcessedMessage[]> {
