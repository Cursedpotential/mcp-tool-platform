    ...
    while (this.queue.length > 0 && this.activeCount < this.concurrencyLimit) {
      ...
      if (item) {
        const task = Array.from(this.tasks.values()).find(
          t =>
            t.contentHash ===
            this.computeInputHash(item.request.toolName, item.request.args)
        );
        if (task) {
  ...
  private registerBuiltinHandlers(): void {
    ...
    this.registerHandler("doc.convert_to_markdown", async args => {
      ...
      return convertToMarkdown(
        args as { path: string; format?: string; extractMetadata?: boolean }
      );
    });

    this.registerHandler("doc.ocr_image_or_pdf", async args => {
      const { ocrImageOrPdf } = await import("../plugins/document");
      return ocrImageOrPdf(
        args as { path: string; language?: string; psm?: number; dpi?: number }
      );
    });
    ...
    this.registerHandler("nlp.extract_keywords", async args => {
      ...
      return extractKeywords(
        args as { textRef: string; method?: string; topK?: number }
      );
    });
    ...
    this.registerHandler("fs.write_file", async args => {
      ...
      return writeFile(
        args as { path: string; contentRef: string; createDirs?: boolean }
      );
    });
    ...
    this.registerHandler("ml.embed", async args => {
      ...
      return generateEmbeddings(
        args as { textRef: string; model?: string; chunkSize?: number }
      );
    });
    ...
    this.registerHandler("summarize.hierarchical", async args => {
      const { hierarchicalSummarize } =
        await import("../plugins/summarization");
    ...
    this.registerHandler("retrieve.supporting_spans", async args => {
      ...
      return retrieveSupportingSpans(
        args as {
          question: string;
          docRef: string;
          topK?: number;
    ...
    this.registerHandler("browser.click", async args => {
      ...
      return click(
        args as { url: string; selector: string; waitForNavigation?: boolean }
      );
    });
    ...
    this.registerHandler("forensics.detect_hurtlex", async args => {
      ...
      const { text, language, categories } = args as {
        text: string;
        language?: string;
        categories?: string[];
      };
    ...
    this.registerHandler("forensics.get_modules", async () => {
      const { patternAnalyzer, BUILT_IN_MODULES } =
        await import("../forensics/pattern-analyzer");
      return {
        modules: BUILT_IN_MODULES,
        activeModules: patternAnalyzer
          .getModules()
          .filter(m => m.enabled)
          .map(m => m.id),
      };
    ...
    this.registerHandler("text.mine", async args => {
      const { mineFiles, generateMarkdownReport } =
        await import("../plugins/text-miner");
    ...
    this.registerHandler("format.convert", async args => {
      const { parseFile, toJson, toCsv, toMarkdown } =
        await import("../plugins/format-converter");
    ...
    this.registerHandler("format.check_schema", async args => {
      ...
      const { inputPath, previewCount } = args as {
        inputPath: string;
        previewCount?: number;
      };
    ...
    this.registerHandler("tesseract.ocr", async args => {
      ...
      return tesseractOcr(
        args as { image: string; language?: string; psm?: number }
      );
    });

    this.registerHandler("stirlingpdf.process", async args => {
      const { stirlingPdfProcess } =
        await import("../plugins/document-processors");
      return stirlingPdfProcess(
        args as {
          operation:
            | "merge"
            | "split"
    ...
    this.registerHandler("unstructured.partition", async args => {
      const { unstructuredPartition } =
        await import("../plugins/document-processors");
      return unstructuredPartition(
        args as { file: string; strategy?: string; extractImages?: boolean }
      );
    });
    ...
    this.registerHandler("schema.apply", async args => {
      const { resolve, applyMapping } =
        await import("../plugins/schema-resolver");
      ...
      const fields =
        sourceFields || (data.length > 0 ? Object.keys(data[0]) : []);
    ...
    this.registerHandler("evidence.create_chain", async args => {
      const { createChainOfCustody } =
        await import("../plugins/evidence-hasher");
    ...
    this.registerHandler("evidence.add_stage", async args => {
      const { addProcessingStage, hashFile, hashContent } =
        await import("../plugins/evidence-hasher");
      const { chain, stage, outputFilePath, outputContent, operator, notes } =
        args as {
          chain: any;
          stage:
            | "imported"
    ...
    this.registerHandler("evidence.generate_report", async args => {
      const { generateForensicReport, verifyChain } =
        await import("../plugins/evidence-hasher");
      const { chain } = args as { chain: any };
    ...
    this.registerHandler("js.xml_parse", async args => {
      ...
      return parseXml(
        args as { xml: string; options?: Record<string, unknown> }
      );
    });
    ...
    this.registerHandler("js.yaml", async args => {
      ...
      return handleYaml(
        args as { input: string; operation?: "parse" | "stringify" }
      );
    });
    ...
    this.registerHandler("llamaindex.chunk_text", async args => {
      ...
      return chunkText(
        args as { text: string; chunkSize?: number; chunkOverlap?: number }
      );
    });
    ...
    this.registerHandler("py.spacy", async args => {
      ...
      return runSpacy(
        args as { text: string; operations?: string[]; model?: string }
      );
    });
    ...
    this.registerHandler("py.pdfplumber", async args => {
      ...
      return runPdfPlumber(
        args as { path: string; pages?: number[]; extractTables?: boolean }
      );
    });
    ...
    this.registerHandler("graphiti.add_entity", async args => {
      ...
      if (!result.success)
        throw new Error(result.error || "Graphiti add_entity failed");
      return result.data;
    });
    ...
    this.registerHandler("graphiti.add_relationship", async args => {
      ...
      if (!result.success)
        throw new Error(result.error || "Graphiti add_relationship failed");
      return result.data;
    });
    ...
    this.registerHandler("graphiti.search_entities", async args => {
      ...
      if (!result.success)
        throw new Error(result.error || "Graphiti search_entities failed");
      return result.data;
    });
    ...
    this.registerHandler("graphiti.get_entity_timeline", async args => {
      ...
      if (!result.success)
        throw new Error(result.error || "Graphiti get_entity_timeline failed");
      return result.data;
    });
    ...
    this.registerHandler("graphiti.detect_contradictions", async args => {
      ...
      if (!result.success)
        throw new Error(
          result.error || "Graphiti detect_contradictions failed"
        );
      return result.data;
    });
    ...
    this.registerHandler("graphiti.query_as_of", async args => {
      ...
      if (!result.success)
        throw new Error(result.error || "Graphiti query_as_of failed");
      return result.data;
    });
