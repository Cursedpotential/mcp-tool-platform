...
export const langGraphTools: ToolSpec[] = [
  {
    ...
    description:
      "Create a new LangGraph workflow definition with nodes, edges, and conditional routing. Returns a graph ID that can be executed.",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        workflow_type: {
          ...
          description:
            'Type of workflow to create. Use "custom" for user-defined workflows.',
        },
        initial_state: {
          type: "object",
    ...
    outputSchema: {
      ...
      properties: {
        graph_id: {
          type: "string",
          description: "Unique identifier for the created graph",
        },
        ...
        checkpoint_count: {
          type: "number",
          description: "Number of human approval checkpoints",
        },
  ...
  },
  {
    ...
    description:
      "Execute a LangGraph workflow and return the final state. Blocks until workflow completes or reaches a checkpoint requiring human approval.",
    category: "orchestration",
    version: "1.0.0",
    ...
    outputSchema: {
      ...
      properties: {
        ...
        status: {
          type: "string",
          enum: ["completed", "checkpoint", "error"],
        ...
        checkpoints_reached: {
          type: "array",
          items: { type: "string" },
          description: "List of checkpoint nodes reached",
        },
      },
    },
  },
  {
    ...
    description:
      "Execute a LangGraph workflow with streaming updates. Returns state snapshots after each node execution for real-time monitoring.",
    category: "orchestration",
    version: "1.0.0",
  ...
  },
  {
    ...
    description:
      "Get the current state of a running or paused workflow. Useful for resuming interrupted workflows or checking checkpoint status.",
    category: "orchestration",
    version: "1.0.0",
    ...
    outputSchema: {
      ...
      properties: {
        ...
        next_checkpoint: {
          type: "string",
          description: "Next checkpoint node (if any)",
        },
      },
    },
  },
  {
    ...
    description:
      "List all available workflow templates with descriptions and capabilities.",
    category: "orchestration",
    version: "1.0.0",
          ...
          items: {
            ...
            properties: {
              ...
              checkpoints: {
                type: "array",
                items: { type: "string" },
                description: "Human approval points",
              },
              required_inputs: {
                type: "array",
                items: { type: "string" },
    ...
    switch (workflow_type) {
      case "forensic_investigation":
        if (!params.evidence_id || !params.case_id) {
          throw new Error(
            "evidence_id and case_id required for forensic_investigation"
          );
        }
      ...
      case "document_processing":
        if (!params.document_id || !params.source_path) {
          throw new Error(
            "document_id and source_path required for document_processing"
          );
        }
      ...
      case "custom":
        ...
        graph = langGraphAdapter
          .createGraph(name, description || "", initial_state)
          .build();
  ...
  "langgraph.executeGraph": async (params: any) => {
    ...
    switch (workflow_type) {
      case "forensic_investigation":
        ...
        result = await executeForensicInvestigation(
          params.evidence_id,
          params.case_id
        );
      ...
      case "document_processing":
        ...
        result = await executeDocumentProcessing(
          params.document_id,
          params.source_path
        );
        checkpointsReached = [];
  ...
  "langgraph.streamGraph": async (params: any) => {
    ...
    switch (workflow_type) {
      case "forensic_investigation":
        ...
        for await (const state of streamForensicInvestigation(
          params.evidence_id,
          params.case_id
        )) {
          snapshots.push({
      ...
      workflows: [
        {
          ...
          description:
            "Multi-stage forensic analysis: preliminary (Chroma, 0-72h) → meta-analysis (Neo4j, post-72h) → reconciliation",
          stages: [
        ...
        },
        {
          ...
          description:
            "Document ingestion pipeline: type detection → extraction → validation → storage (R2/Supabase/Directus)",
          stages: [
            "type_detection",
            "content_extraction",
            "validation",
