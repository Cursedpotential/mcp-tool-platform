...
async function ocrImage(
  ...
  if (!tools.tesseract) {
    throw new Error(
      "Tesseract not installed. Install with: apt install tesseract-ocr"
    );
  }
...
async function ocrPdf(filePath: string, lang: string = "eng"): Promise<string> {
  ...
  try {
    ...
    await execAsync(`pdftoppm -png "${filePath}" "${tmpDir}/page"`, {
      timeout: 120000,
    });
...
function parseHtml(content: string): Message[] {
  ...
  const msgPattern =
    /<div[^>]*class="[^"]*(?:message|msg|chat)[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
  let match;
...
async function parseFile(
  filePath: string
): Promise<{ messages: Message[]; format: string }> {
...
async function checkSchema(
  ...
  try {
    ...
    for (const msg of messages) {
      ...
      if (msg.contactName || msg.contact_name || msg.sender || msg.from)
        stats.contactName++;
...
function toCsv(messages: Message[]): string {
  ...
  const rows = messages.map(m => {
    return fields
      .map(f => {
        const val = String(
          m[f] || m[f.replace(/([A-Z])/g, "_$1").toLowerCase()] || ""
        )
          .replace(/"/g, '""')
          .replace(/\n/g, " ")
          .slice(0, 500);
...
function toMarkdown(messages: Message[]): string {
  const lines = ["# Exported Messages", `*${messages.length} messages*`, ""];

  for (const m of messages) {
    const date = m.readableDate || m.readable_date || m.date || "N/A";
    const sender =
      m.contactName || m.contact_name || m.sender || m.address || "Unknown";
    const body = m.body || "";
...
export const formatConverterTools = [
  {
    name: "convert.parse",
    description:
      "Parse any supported file format into structured messages. Supports JSON, CSV, HTML, PDF, images (OCR), DOCX.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Validate file schema and check field coverage before processing",
    inputSchema: {
      type: "object",
      properties: {
        ...
        previewCount: {
          type: "number",
          description: "Number of sample messages",
          default: 5,
        },
      },
      required: ["filePath"],
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        outputFormat: {
          type: "string",
          enum: ["json", "csv", "md", "html", "pdf", "docx"],
    ...
    handler: async (params: {
      ...
      switch (params.outputFormat) {
        ...
        case "docx":
          ...
          const converted = await convertWithPandoc(
            md,
            params.outputFormat as any
          );
          if (!converted) {
  ...
  },
  {
    name: "convert.status",
    description:
      "Check availability of conversion tools (Tesseract, Pandoc, etc.)",
