...
export const schemaResolverTools = [
  {
    ...
    inputSchema: z.object({
      sourceFields: z.array(z.string()).describe('Field names from source data'),
      sample: z.array(z.record(z.unknown())).optional().describe('Sample records for content analysis'),
      useCache: z.boolean().default(true).describe('Use cached mappings'),
      useAi: z.boolean().default(false).describe('Allow AI resolution as fallback')
    }),
  ...
  },
  {
    ...
    inputSchema: z.object({
      data: z.array(z.record(z.unknown())).describe('Data to transform'),
      mappings: z.array(z.object({
        sourceField: z.string(),
        targetField: z.string()
      })).describe('Field mappings to apply')
    }),
    handler: async (params: any) => {
  ...
  },
  {
    ...
    permissions: ['schema.write'],
    inputSchema: z.object({
      data: z.array(z.record(z.unknown())).describe('Raw data to resolve and transform'),
      useAi: z.boolean().default(false).describe('Allow AI resolution')
    }),
    handler: async (params: any) => {
      ...
      const { mappings, method } = await resolve(sourceFields, params.data, true, params.useAi);
  ...
  },
  {
    ...
    permissions: ['schema.read'],
    inputSchema: z.object({}),
    handler: async (params: any) => getCacheStats()
  },
  {
...
// Standalone functions for direct gateway calls
export const schemaResolver = {
  async detectSchema(data: any) {
    // Find the resolve tool and call its handler
    const resolveTool = schemaResolverTools.find(t => t.name === 'schema.resolve');
    if (!resolveTool?.handler) {
      throw new Error('Schema resolve tool not found');
    }
    return await resolveTool.handler({ sourceFields: Object.keys(data), sample: [data], useCache: true, useAi: false });
  },

  async autoResolve(data: any) {
    // Find the auto_resolve tool and call its handler
    const resolveTool = schemaResolverTools.find(t => t.name === 'schema.auto_resolve');
    if (!resolveTool?.handler) {
      throw new Error('Schema auto_resolve tool not found');
    }
    return await resolveTool.handler({ data: Array.isArray(data) ? data : [data], useAi: false });
  }
};
