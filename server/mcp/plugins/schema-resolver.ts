    ...
    for (const src of sourceFields) {
      ...
      for (const alias of aliases) {
        ...
        if (srcLower.includes(aliasLower) || aliasLower.includes(srcLower)) {
          const score =
            Math.min(alias.length, src.length) /
            Math.max(alias.length, src.length);
...
export const schemaResolverTools = [
  {
    name: "schema.resolve",
    description:
      "Resolve unknown field names to standard schema (body, date, contactName, address). Uses heuristics first, AI as last resort.",
    inputSchema: {
      type: "object",
      properties: {
        sourceFields: {
          type: "array",
          items: { type: "string" },
          description: "Field names from source data",
        },
        sample: {
          type: "array",
          items: { type: "object" },
          description: "Sample records for content analysis",
        },
        useCache: {
          type: "boolean",
          description: "Use cached mappings",
          default: true,
        },
        useAi: {
          type: "boolean",
          description: "Allow AI resolution as fallback",
          default: false,
        },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        data: {
          type: "array",
          items: { type: "object" },
          description: "Raw data to resolve and transform",
        },
        useAi: {
          type: "boolean",
          description: "Allow AI resolution",
          default: false,
        },
      },
      required: ["data"],
    ...
    handler: async (params: {
      ...
      const { mappings, method } = await resolve(
        sourceFields,
        params.data,
        true,
        params.useAi || false
      ...
      if (!mappings.length) {
        return {
          success: false,
          error: "Could not resolve schema",
