...
export async function tesseractOcr(args: {
  ...
  try {
    const result = await ocrImageOrPdf({
      path: imagePath,
...
export async function stirlingPdfProcess(args: {
  ...
  if (args.operation !== "ocr") {
    const baseUrl =
      typeof args.options?.baseUrl === "string"
        ? String(args.options.baseUrl)
        : process.env.STIRLINGPDF_URL;

    if (!baseUrl) {
      throw new Error(
        "StirlingPDF base URL not configured for non-OCR operations"
      );
    }

    const endpoint =
      typeof args.options?.endpoint === "string"
