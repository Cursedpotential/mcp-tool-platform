 ...
 */
export function configureSummarization(
  newConfig: Partial<SummarizationConfig>
): void {
...
export async function hierarchicalSummarize(
  args: HierarchicalSummarizeArgs
): Promise<{
  ...
  return {
    ...
    compressionRatio,
    preview:
      finalSummary.slice(0, 300) + (finalSummary.length > 300 ? "..." : ""),
  };
}
...
export async function generateOutline(args: { textRef: string }): Promise<{
  ...
  while ((match = headingRegex.exec(text)) !== null) {
    ...
    const content = text
      .slice(match.index + match[0].length, contentEnd)
      .trim();
...
function splitIntoChunks(text: string, maxSize: number): string[] {
  ...
  for (const para of paragraphs) {
    if (
      currentChunk.length + para.length + 2 > maxSize &&
...
function scoreByTextRank(
  ...
  const wordSets = sentences.map(
    s => new Set(s.toLowerCase().match(/\b\w{3,}\b/g) ?? [])
  );
    ...
    for (let j = 0; j < sentences.length; j++) {
      if (i !== j) {
        const intersection = new Set(
          Array.from(wordSets[i]).filter(w => wordSets[j].has(w))
        );
        const union = new Set([
...
function scoreByPosition(
  ...
  return sentences.map((text, index) => {
    let score = 0;
    if (index < 3)
      score = 1 - index * 0.2; // First sentences
    else if (index >= sentences.length - 2)
      score = 0.6; // Last sentences
