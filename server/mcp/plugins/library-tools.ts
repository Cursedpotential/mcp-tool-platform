...
export async function parseJson5(args: {
  text: string;
}): Promise<{ data: unknown }> {
...
function normalizeCsvRows(
  ...
  if (Array.isArray(input)) {
    ...
    if (
      ...
      const columns =
        (options?.columns as string[] | undefined) ??
        Array.from(
          new Set(
            input.flatMap(item => Object.keys(item as Record<string, unknown>))
          )
        );
...
export async function runNatural(args: {
  ...
  switch (args.operation) {
    ...
    case "classify": {
      ...
      const training =
        (args.options?.training as Array<{ text: string; label: string }>) ??
        [];
...
export async function detectFranc(args: {
  ...
  const normalized =
    ranked.length > 0
      ? 1 - score / (ranked[ranked.length - 1]?.[1] ?? score)
      : 0;
  return {
    language: lang,
    confidence: Number.isFinite(normalized) ? normalized : 0,
  };
}
...
export async function compareStrings(args: {
  ...
  if (algorithm === "jaro-winkler") {
    return {
      similarity: natural.JaroWinklerDistance(args.string1, args.string2),
    };
  }
  ...
  return {
    similarity: stringSimilarity.compareTwoStrings(args.string1, args.string2),
  };
}
