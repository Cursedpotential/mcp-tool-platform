...
function detectContentType(paths: string[]): ContentType {
  ...
  const conversationExtensions = new Set([
    ".json",
    ".csv",
    ".tsv",
    ".txt",
  ...
  if (conversationCount > codeCount && conversationCount > documentCount)
    return "conversation";
  if (documentCount > codeCount && documentCount > conversationCount)
    return "document";
  return "mixed";
}
...
async function selectEngine(
  ...
  if (preferredEngine === "auto") {
    ...
    if (
      (contentType === "conversation" || contentType === "document") &&
      ugrepAvailable
    ) {
      return "ugrep";
    }
...
export const textMinerTools = [
  {
    ...
    description:
      "Fast forensic text search with smart engine routing (ugrep for conversations, ripgrep for code). Extracts timestamps and generates timeline.",
    inputSchema: {
      type: "object",
      properties: {
        searchTerm: {
        ...
        regexMode: {
          type: "boolean",
          description: "Treat searchTerm as regex",
          default: false,
        },
        fileTypes: {
          type: "array",
  ...
  },
  {
    name: "search.smart",
    description:
      "Auto-select best search engine based on content type and execute search",
    inputSchema: {
      type: "object",
      properties: {
      ...
      const engine =
        params.type === "code"
          ...
          : params.type === "conversation"
            ? "ugrep"
            : "auto";
      return mineFiles(params.query, [params.path], {
        recursive: true,
