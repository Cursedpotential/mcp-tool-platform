...
export function registerNLPHandlers(app: any) {
  app.post(
    "/gcp-natural-language/analyzeSentiment",
    async (req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to analyze sentiment." });
      }
    }
  );

  app.post(
    "/gcp-natural-language/analyzeEntities",
    async (req: Request, res: Response) => {
      try {
        const { text } = req.body;
        if (!text) {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to analyze entities." });
      }
    }
  );

  app.post(
    "/gcp-natural-language/analyzeSyntax",
    async (req: Request, res: Response) => {
      try {
        const { text } = req.body;
        if (!text) {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to analyze syntax." });
      }
    }
  );

  app.post(
    "/gcp-natural-language/classifyText",
    async (req: Request, res: Response) => {
      try {
        const { text } = req.body;
        if (!text) {
        ...
        res
          .status(500)
          .json({ error: error.message || "Failed to classify text." });
      }
    }
  );
}
