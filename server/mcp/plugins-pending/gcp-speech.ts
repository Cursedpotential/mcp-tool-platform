...
const transcribeAudio: PluginHandler<
...
const transcribeLongAudio: PluginHandler<
  TranscribeLongAudioRequest,
  TranscribeLongAudioResponse
> = async request => {
  const audio = {
    uri: request.gcsUri,
  };
...
const getLongAudioTranscription: PluginHandler<
  GetLongAudioTranscriptionRequest,
  GetLongAudioTranscriptionResponse
> = async (request: GetLongAudioTranscriptionRequest) => {
  ...
  if (operation.done) {
    ...
    const transcription = operation.response?.results
      ...
      .join("\n");
    const confidence =
      operation.response?.results?.[0]?.alternatives?.[0]?.confidence || 0;
