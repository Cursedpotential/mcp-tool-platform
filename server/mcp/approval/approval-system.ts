...
async function requestApproval(
  ...
  await savePending();
  await logAudit(approvalId, "created", context.requestedBy, {
    operation,
    target: plan.target,
  });
...
async function approve(
  ...
  if (request.status !== "pending") {
    throw new Error(
      `Approval ${approvalId} is ${request.status}, cannot approve`
    );
  }
...
async function reject(
  ...
  if (request.status !== "pending") {
    throw new Error(
      `Approval ${approvalId} is ${request.status}, cannot reject`
    );
  }
...
async function rollback(
  ...
  if (!request.rollback.available || !request.rollback.backupPath) {
    return {
      success: false,
      message: "Rollback not available for this operation",
    };
  }
  ...
  try {
    ...
    await savePending();
    await logAudit(approvalId, "rolled_back", rolledBackBy, {
      backupPath: request.rollback.backupPath,
    });

    return {
      success: true,
      message: `Rolled back ${request.plan.target} from backup`,
    };
  } catch (error: any) {
...
export const approvalTools = [
  {
    ...
    description:
      "Request human approval for a destructive/sensitive operation. Returns approval_id that must be approved before execution.",
    inputSchema: {
      type: "object",
      properties: {
        operation: {
        ...
        previewType: {
          type: "string",
          enum: ["diff", "summary", "list"],
          default: "summary",
        },
        previewBefore: {
          type: "string",
          description: "Content before change (for diff)",
        },
        previewAfter: {
          type: "string",
          description: "Content after change (for diff)",
        },
    ...
    handler: async (params: any) => {
      ...
      if (
        ["file_write", "file_delete", "file_move"].includes(params.operation)
      ) {
        try {
      ...
      if (
        params.previewType === "diff" &&
        params.previewBefore &&
        params.previewAfter
      ) {
