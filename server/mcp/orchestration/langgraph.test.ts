...
describe("LangGraph Adapter", () => {
  describe("Graph Builder", () => {
    ...
    it("should add nodes to graph", () => {
      const graph = langGraphAdapter.createGraph("test", "Test", {
        stage: "init",
      });
    ...
    it("should set entry point", () => {
      ...
      graph.addNode("start", async state => state).setEntryPoint("start");
  ...
  describe("Graph Execution", () => {
    ...
    it("should execute graph with conditional routing", async () => {
      const graph = langGraphAdapter.createGraph<{
        value: number;
        path: string;
      }>("router", "Router graph", { value: 75, path: "" });
...
describe("Graph State Management", () => {
  ...
  it("should return null for non-existent workflow", async () => {
    const retrieved = await langGraphAdapter.getGraphState(
