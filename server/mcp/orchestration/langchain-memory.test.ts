...
describe("ForensicInvestigationMemory", () => {
  ...
  describe("Hypothesis Recording", () => {
    it("should record preliminary hypothesis", async () => {
      ...
      expect(vars.preliminary_hypotheses[0].hypothesis).toContain(
        "gaslighting"
      );
    ...
    it("should track evidence references", async () => {
      ...
      const vars = await memory.loadMemoryVariables();
      expect(vars.preliminary_hypotheses[0].evidence_refs).toEqual(
        evidenceRefs
      );
    });
  });
  ...
  describe("Hypothesis Evolution", () => {
    ...
    it("should mark superseded hypotheses", async () => {
      ...
      const updatedHyp1 = vars2.preliminary_hypotheses.find(
        h => h.id === hyp1Id
      );
  ...
  describe("Analysis Delta Recording", () => {
    ...
    it("should log significant contradictions", async () => {
      ...
      const reasoning = await memory.getReasoningTrail();
      expect(reasoning.some(r => r.includes("Contradiction detected"))).toBe(
        true
      );
    ...
    it("should track human annotations", async () => {
      const delta: AnalysisDelta = {
        ...
        human_annotation: {
          investigator_note:
            "Classic gaslighting pattern confirmed by forensic psychologist",
          methodology_justification:
            "Cross-referenced with DSM-5 criteria and established case law",
      ...
      expect(allDeltas[0].human_annotation?.evidence_admissibility).toBe(
        "conclusive"
      );
    });
  });
  ...
  describe("Memory Export", () => {
    it("should export complete memory for forensic report", async () => {
      await memory.recordHypothesis("Test hypothesis 1", "preliminary", 0.7, [
        "msg_001",
      ]);
      await memory.recordHypothesis("Test hypothesis 2", "full_context", 0.9, [
        "msg_001",
        "msg_002",
      ]);
