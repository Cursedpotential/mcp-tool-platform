...
import {
  priorityScreener,
  type PriorityScreenResult,
} from "./priority-screener";
...
export class MultiPassClassifier {
  ...
  constructor() {
    this.nlpRunnerPath = path.join(
      __dirname,
      "../../python-tools/nlp_runner.py"
    );
  }
  ...
  async loadUserPatterns(userId: number): Promise<void> {
    ...
    console.log(
      `[MultiPassClassifier] Loading custom patterns for user ${userId}`
    );
  ...
  async classify(
    ...
    console.log(
      `[MultiPassClassifier] Starting multi-pass analysis (${text.length} chars)`
    );
    ...
    // ========================================================================
    console.log(
      "[MultiPassClassifier] Pass 0: Priority screening (custody/alienation)"
    );
    ...
    if (priorityResult.has_priority_flags) {
      console.log(
        `[MultiPassClassifier] 🚨 PRIORITY: ${priorityResult.flags.length} critical flags detected`
      );
    }
    ...
    try {
      console.log(
        "[MultiPassClassifier] Pass 1: spaCy (structure & attribution)"
      );
      ...
      const personEntities =
        entities.entities?.filter((e: any) => e.type === "PERSON") || [];
      ...
      result.linguistic.imperative_detected =
        /^(do|don't|stop|give|tell|show|let)/i.test(text.trim());
    ...
    try {
      console.log(
        "[MultiPassClassifier] Pass 2: NLTK VADER (sentiment & negation)"
      );
      ...
      result.linguistic.negation_detected = negationWords.some(w =>
      ...
      result.linguistic.intensity_modifiers = intensifiers.filter(w =>
        text.toLowerCase().includes(w)
      );
    ...
    try {
      console.log(
        "[MultiPassClassifier] Pass 3: Pattern Analyzer (custom patterns)"
      );
      ...
      if (
        patternResult.negativeMatches.length >
        patternResult.positiveMatches.length
      ) {
    ...
    try {
      console.log(
        "[MultiPassClassifier] Pass 4: TextBlob (polarity & sarcasm)"
      );
...
`);
      ...
      if (textblobResult.polarity !== undefined) {
        result.sentiment.polarity =
          (result.sentiment.polarity + textblobResult.polarity) / 2;
      }
    ...
    try {
      console.log(
        "[MultiPassClassifier] Pass 5: Sentence Transformers (semantic matching)"
      );
    ...
    try {
      ...
      const keywordsResult = await this.runPython("extract_keywords", {
        text,
        topK: 10,
      });
    ...
    result.severity = Math.max(
      computedSeverity,
      result.priority.immediate_severity
    );
    ...
    console.log(
      `[MultiPassClassifier] Complete: ${result.sentiment.label} (severity: ${result.severity}, confidence: ${result.confidence.toFixed(2)})`
    );
  ...
  private async runPython(command: string, args: any): Promise<any> {
    try {
      ...
      console.error(
        `[MultiPassClassifier] Python command '${command}' failed:`,
        error.message
      );
      return {};
    }
  ...
  private async runPythonScript(script: string): Promise<any> {
    try {
      ...
      console.error(
        "[MultiPassClassifier] Python script failed:",
        error.message
      );
      return {};
    }
  ...
  private computeConfidence(
    result: MultiPassClassification,
    sourceCount: number
  ): number {
  ...
  async classifyBatch(
    ...
    Array<{ chunk_id: string; classification: MultiPassClassification }>
  > {
    console.log(
      `[MultiPassClassifier] Batch classifying ${chunks.length} chunks`
    );

    const results: Array<{
      chunk_id: string;
      classification: MultiPassClassification;
    }> = [];
    ...
    for (const chunk of chunks) {
      try {
        ...
        console.error(
          `[MultiPassClassifier] Failed chunk ${chunk.chunk_id}:`,
          error.message
        );
      }
    }
