...
export class Classifier {
  ...
  async classify(text: string, context?: string): Promise<Classification> {
    ...
    try {
      ...
      console.log(
        `[Classifier] Result: ${classification.sentiment} (severity: ${classification.severity}, patterns: ${classification.patterns.length})`
      );
  ...
  async classifyBatch(
    ...
    for (const chunk of chunks) {
      try {
        ...
        console.error(
          `[Classifier] Failed to classify chunk ${chunk.chunk_id}:`,
          error.message
        );
        results.push({
          chunk_id: chunk.chunk_id,
  ...
  async classifyBatchParallel(
    ...
    console.log(
      `[Classifier] Classifying ${chunks.length} chunks (parallel, concurrency: ${concurrency})`
    );
    ...
    for (let i = 0; i < chunks.length; i++) {
      const promise = (async (index: number) => {
        try {
          ...
          console.error(
            `[Classifier] Parallel classification ${index} failed:`,
            error.message
          );
          results[index] = {
            chunk_id: chunks[index].chunk_id,
 ...
 */
export function aggregateClassifications(classifications: Classification[]): {
