...
import type {
  PatternMatch,
  AnalysisResult,
} from "../forensics/pattern-analyzer";
...
export class NLPClassifier {
  ...
  private async analyzeSentiment(
    text: string
  ): Promise<NLPClassification["sentiment"]> {
    try {
      ...
      console.error(
        "[NLPClassifier] Sentiment analysis failed:",
        error.message
      );
      return {
        label: "neutral",
  ...
  private async extractEntities(
    text: string
  ): Promise<NLPClassification["entities"]> {
    try {
  ...
  private async extractKeywords(
    text: string
  ): Promise<NLPClassification["keywords"]> {
    try {
      ...
      console.error(
        "[NLPClassifier] Keyword extraction failed:",
        error.message
      );
      return [];
    }
  ...
  private computeSeverity(patterns: {
    ...
    const avgSeverity =
      patterns.negativeMatches.reduce((sum, m) => sum + (m.severity || 5), 0) /
      patterns.negativeMatches.length;
  ...
  private computeConfidence(
    ...
    const totalMatches =
      patterns.negativeMatches.length + patterns.positiveMatches.length;
  ...
  async classifyBatch(
    ...
    for (const chunk of chunks) {
      try {
        ...
        console.error(
          `[NLPClassifier] Failed to classify chunk ${chunk.chunk_id}:`,
          error.message
        );
...
export function aggregateNLPClassifications(
  ...
  for (const c of classifications) {
    sentimentCounts[c.sentiment.label] =
      (sentimentCounts[c.sentiment.label] || 0) + 1;
