export default {
  async fetch(request, env) {
    ...
    if (method === "POST" && path.startsWith("/webhook/")) {
      ...
      if (!source) {
        return new Response("Missing source in path", {
          status: 400,
          headers: corsHeaders(),
        });
      }
      ...
      try {
        ...
        env.KV_NAMESPACE.put(
          `webhook:${id}`,
          JSON.stringify({ ...webhookRecord, status: "processing" })
        )
          .then(() => forwardWebhookToBackend(webhookRecord, env))
          .catch(error =>
            console.error(`Error forwarding webhook ${id}:`, error)
          );

        return new Response(
          JSON.stringify({ id, message: "Webhook received and queued" }),
          {
            status: 202,
        ...
        return new Response(
          `Invalid JSON or internal error: ${error.message}`,
    ...
    if (method === "POST" && path.match(/^\/webhook\/[^/]+\/retry$/)) {
      ...
      if (!id) {
        return new Response("Missing webhook ID", {
          status: 400,
          headers: corsHeaders(),
        });
      }
      ...
      return new Response(
        JSON.stringify({ id, message: "Webhook retry initiated" }),
        {
          status: 202,
          headers: { ...corsHeaders(), "Content-Type": "application/json" },
        }
...
async function forwardWebhookToBackend(webhookRecord, env) {
  try {
    ...
    if (!backendUrl) {
      console.error(
        "BACKEND_WEBHOOK_URL is not defined in environment variables."
      );
    ...
    if (response.ok) {
      ...
      console.error(
        `Failed to forward webhook ${webhookRecord.id} to backend: ${response.status} ${response.statusText}`
      );
    }
  } catch (error) {
