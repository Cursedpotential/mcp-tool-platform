...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === "/health") {
      return new Response(
        JSON.stringify({ status: "healthy", service: "auth-proxy" }),
        {
...
function getRequiredPermission(path, method) {
  ...
  if (path.startsWith("/api/evidence"))
    return writeOps.includes(method) ? "evidence:write" : "evidence:read";
  if (path.startsWith("/api/documents"))
    return writeOps.includes(method) ? "documents:write" : "documents:read";
