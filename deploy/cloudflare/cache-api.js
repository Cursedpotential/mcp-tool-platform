  ...
  try {
    ...
    if (!key || value === undefined) {
      return addCorsHeaders(
        new Response("Missing key or value", { status: 400 })
      );
    }
    ...
    return addCorsHeaders(
      new Response(`Error setting cache: ${error.message}`, { status: 500 })
    );
  }
}
...
async function handleGet(request, env, key) {
  try {
    ...
    return addCorsHeaders(
      new Response(`Error getting cache: ${error.message}`, { status: 500 })
    );
  }
}
...
async function handleLlmCache(request, env) {
  try {
    ...
    if (!prompt || !model || response === undefined) {
      return addCorsHeaders(
        new Response("Missing prompt, model, or response", { status: 400 })
      );
    }
    ...
    return addCorsHeaders(
      new Response(`Error handling LLM cache: ${error.message}`, {
        status: 500,
      })
    );
  }
